...
// Create a first sheet
    ...
    $objPHPExcel->getActiveSheet()->setTitle('MTD Complete Data');
...
//    $today_dt = date('Y-m-d',strtotime('-1 days'));
    $today_dt = date('Y-m-d');
    $srt_dt = date('Y-m-01');
    $st_dt = $srt_dt . " 00:00:00 ";
    $st_dt_db = $db->safe(trim($st_dt));
    $ed_dt = $today_dt . " 23:59:59 ";
//    $ed_dt = $today_dt . " 12:59:59 ";
    $ed_dt_db = $db->safe(trim($ed_dt));

	//$st_dt_db = $db->safe('2018-12-01 00:00:00');
	//$ed_dt_db = $db->safe('2018-12-31 23:59:59');
...
//$query="select ip.invoice_no, ip.invoice_date, ip.delivery_date, ip.expiry_date, ip.tqty, ip.tamt,ip.ctime as Intouch, ip.shipping_id as sid,ipt.mrp, ipt.qty,ipt.tot_qty,ipt.pack_type as CAR,ipt.cost_price,ipt.vat, ipt.amt, ipt.master_item_id ,imd.name as dealername,imd.show_code as showflag, idt.itemcode as articleno,idt.itemname as description,  d.supplier_id as distid, d.name as distname, sh.dc_address as address,sh.dc_state as state,sh.dc_city as city,sh.dc_name as name, bu.code as vendorcode,ip.invoice_text,sh.customer_code from  it_master_dealers imd, it_dealer_items idt,it_shipping_address sh, it_distributors d, it_business_unit bu, it_po ip, it_po_items ipt  where ip.id=ipt.po_id  AND  ip.dist_id=d.id and d.bu_id= bu.id and sh.id = ip.shipping_id  and  ip.master_dealer_id=imd.id AND idt.id= ipt.dealer_item_id and idt.is_vlcc = 1  AND ip.ctime between $st_dt_db and $ed_dt_db and ip.status =1 order by dealername,ip.invoice_no;";
 //$query="select ip.invoice_no, ip.invoice_date, ip.delivery_date, ip.expiry_date, ip.tqty, ip.tamt,ip.ctime as Intouch, ip.shipping_id as sid,ipt.mrp, ipt.qty,ipt.tot_qty,ipt.pack_type as CAR,ipt.cost_price,ipt.vat, ipt.amt, ipt.master_item_id ,imd.name as dealername,imd.show_code as showflag, idt.itemcode as articleno,idt.itemname as description,  d.code as distid, d.name as distname, sh.dc_address as address,sh.dc_state as state,sh.dc_city as city,sh.dc_name as name, bu.code as vendorcode,ip.invoice_text,sh.customer_code from  it_master_dealers imd, it_dealer_items idt,it_shipping_address sh, it_distributors d, it_business_unit bu, it_po ip, it_po_items ipt  where ip.id=ipt.po_id  AND  ip.dist_id=d.id and d.bu_id= bu.id and sh.id = ip.shipping_id  and  ip.master_dealer_id=imd.id AND idt.id= ipt.dealer_item_id and idt.is_vlcc = 1  AND ip.ctime between $st_dt_db and $ed_dt_db and ip.status =1 and imd.id not in (11,7) order by dealername,ip.invoice_no;"; 
//   $query="select ip.invoice_no, ip.invoice_date, ip.delivery_date, ip.expiry_date, ip.tqty, ip.tamt,ip.ctime as Intouch, ip.shipping_id as sid,ipt.mrp, ipt.qty,ipt.tot_qty,ipt.pack_type as CAR,ipt.cost_price,ipt.vat, ipt.amt, ipt.master_item_id ,imd.name as dealername,imd.show_code as showflag, idt.itemcode as articleno,idt.itemname as description,idt.eancode,  d.code as distid, d.name as distname, sh.dc_address as address,sh.dc_state as state,sh.dc_city as city,sh.dc_name as name, bu.code as vendorcode,ip.invoice_text,sh.customer_code,imd.id as master_deaer_id from  it_master_dealers imd, it_dealer_items idt,it_shipping_address sh, it_distributors d, it_business_unit bu, it_po ip, it_po_items ipt where ip.id=ipt.po_id AND ip.dist_id=d.id and d.bu_id= bu.id and sh.id = ip.shipping_id  and  ip.master_dealer_id=imd.id AND idt.id= ipt.dealer_item_id AND ip.ctime between $st_dt_db and $ed_dt_db and ip.status =1 and imd.id not in (27) and ip.is_active=1 order by dealername,ip.invoice_no"; 
     $query="select ip.invoice_no, ip.invoice_date, ip.delivery_date, ip.expiry_date, ip.tqty, ip.tamt,ip.ctime as Intouch, ip.shipping_id as sid,ipt.mrp, ipt.qty,ipt.tot_qty,ipt.pack_type as CAR,ipt.cost_price,ipt.vat, ipt.amt, ipt.master_item_id ,imd.name as dealername,imd.show_code as showflag, idt.itemcode as articleno,idt.itemname as description,  d.code as distid, d.name as distname, sh.dc_address as address,sh.dc_state as state,sh.dc_city as city,sh.dc_name as name, bu.code as vendorcode,ip.invoice_text,sh.customer_code,imd.id as master_dealer_id,ipt.po_itemname as description_po,idt.is_vlcc,ipt.po_eancode from  it_master_dealers imd, it_dealer_items idt,it_shipping_address sh, it_distributors d, it_business_unit bu, it_po ip, it_po_items ipt where ip.id=ipt.po_id AND ip.dist_id=d.id and d.bu_id= bu.id and sh.id = ip.shipping_id  and  ip.master_dealer_id=imd.id AND idt.id= ipt.dealer_item_id AND ip.ctime between $st_dt_db and $ed_dt_db and ip.status not in (10,3) and ip.is_active=1 order by dealername,ip.invoice_no"; 
    echo $query."<br/>";
...
//        if(stripos($obj->distname,"vlcc")==TRUE){
	...
	if(trim($Exp_date[0])=='0000-00-00' && $obj->master_dealer_id==8){
        ...
        if(trim($obj->vat)<=0){
         ...
         if($obj->master_dealer_id==3 || $obj->master_dealer_id==4 || $obj->master_dealer_id==5 || $obj->master_dealer_id==41 || $obj->master_dealer_id==49 || $obj->master_dealer_id==24 || $obj->master_dealer_id==8){
            $amt = $obj->amt;
        }else{
...
// Create a first sheet
    $objPHPExcel->setActiveSheetIndex($sheetIndex);
    $objPHPExcel->getActiveSheet()->setTitle('MTD VLCC Data');
    $objPHPExcel->getActiveSheet()->setCellValue('A1', 'Sr.No');
    $objPHPExcel->getActiveSheet()->setCellValue('B1', 'PO Number');
    $objPHPExcel->getActiveSheet()->setCellValue('C1', 'PO Date');
    $objPHPExcel->getActiveSheet()->setCellValue('D1', 'PO Expiry Date');
