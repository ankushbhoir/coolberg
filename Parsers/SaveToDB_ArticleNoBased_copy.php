...
function printItemsArticleNoBased($header,$items,$invtext,$iniid,$filename,$shipping_address,$shipping_id,$rows) {
    print"<br>IN SAVETODB_ARTICLENOBASED_copy.PHP<br>";
    //$parse_dttm = date('dd/mm/yyyy h:i:s a');
    $parse_dttm = date('d-M-Y h:i:s a');
    $resp = printItemsToConsoleArticleNoBased($header,$items,$invtext,$iniid,$filename,$shipping_address,$shipping_id,$rows);
    return $resp;
}

function printItemsToConsoleArticleNoBased($header,$items,$invtext,$iniid,$filename,$shipping_address,$shipping_id,$rows) {
    $db = new DBConn();
    echo "PO header detail";
    print_r($header); 
    $parse_dttm = date('d/M/Y h:i:s a');
    $docItems = array();
    $matches=array();
...
//                 print_r($myDateTime);
    ...
    if(trim($header->Type)==""){
     ...
     // print"<br>tin1**********=$header->Vat_Tin<br>"; 
     // exit;
  ...
  // print "<br>MDQRY: $q<br>";
   ...
   // if($masterdealerid==11 || $masterdealerid==3 || $masterdealerid==4 || $masterdealerid==7){
    ...
    if(trim($header->DealerCode)==""){
       ...
       echo $Dealer_code=trim($header->DealerCode);
       //exit;
        // code by mayur for dmart. took customer pincode as dealer code
        // if($dealerinfo->id == 54){
        //     $Dealer_pincode = substr(trim($shipping_address), -6);
        //     if(is_numeric($Dealer_pincode)){
        //         $Dealer_code = $Dealer_pincode;
        //     }
        // }
...
//        print_r($item);
         ...
         $docItems[] = array(
                   ...
                   "TQty" => ($totQty),
                   "HSN" => trim(getFieldValue($item,"HSN")),
                   "MRP" => ( $mrp),
...
//commeting following block as ttk dont want invoice no. append with dealer code ... #by mayur 06082020
 ...
 //    }
       ...
       echo "<br>$selbuQry";
        ...
        $Dealer_code_db="";
         if(isset($Dealer_code) && trim($Dealer_code)!=""){
                ...
                $Dealer_code_db = "NULL";
                 if($masterdealerid==11){
                    preg_match('/\b(\d[\- ]*){6}\b(?<! )/', $shipping_address_db,$pincode);
                   
                   
                    $Dealer_code_db= $pincode[0];
                }
                         if($masterdealerid==3){
                    
                   
                   
                    $Dealer_code_db= $no_spaces_db;
                }
            }
        if(isset($buinfo)){
                $buid = $buinfo->id;
    ...
    //echo "<br> Dist Query $qdist";
    $distinfo = $db->fetchObject("$qdist");//get distributor id if not then insert distributor
    if(isset($distinfo)){
            $distid = $distinfo->id;
            if(!isset($distinfo->name) || $distinfo->name==""){
                $uquery="update it_distributors set name=$dist where bu_id= $buid $addClause";
                   ...
                   // exit;
                        ...
                        if(isset($item["EAN"])){
                            $EAN = trim(getFieldValue($item,"EAN"));
                             ...
                             if(isset($item["SGST"])){
                            $SGST = trim(getFieldValue($item,"SGST"));
                        }
                              if(isset($item["CGST"])){
                            $CGST = trim(getFieldValue($item,"CGST"));
                        }
                                if(isset($item["IGST"])){
                            $IGST = trim(getFieldValue($item,"IGST"));
                        }
                       ...
                       // $VAT = trim(getFieldValue($item,"VAT"));
                        ...
                        $ttk_qty = doubleval(str_replace(",","",trim(getFieldValue($item,"ttk_qty"))));
                        $ttk_CAR = trim(getFieldValue($item,"ttk_CAR"));
                        $ttk_hsn = trim(getFieldValue($item,"HSN"));
                        $ttk_hsn_db = $db->safe($ttk_hsn);
                         
                      //  print"<br>itemname=$itemname_po<br>";
                        if($masterdealerid==8){

                            $ttk_qty = doubleval(str_replace(",","",trim(getFieldValue($item,"Qty"))));
                             $ttk_CAR = trim(getFieldValue($item,"CAR"));
                        }
                             if($masterdealerid==15){

                            echo $ttk_qty = doubleval(str_replace(",","",trim(getFieldValue($item,"Qty"))));
                             $ttk_CAR = trim(getFieldValue($item,"CAR"));
                        }
...
//                                $norowupdff = $db->execUpdate($uquery);
                            ...
                            if($poid>0 && $dealer_item_id>0){// && $master_item_id>0){                                 
                                ...
                                $ttk_car_db = $db->safe(trim($ttk_CAR));
                  ...
                  $itemname_db = $db->safe($itemname_po);//                                                              
                                // echo "here";
                                // exit;
                                $qpoitem="INSERT INTO it_po_items set po_id = $poid,po_itemname=$itemname_db,dealer_item_id =  $dealer_item_id $eanaddClause $mas_itm_clause ,mrp = $MRP,qty = $Qty, tot_qty = $totQty,po_hsn=$ttk_hsn_db ,pack_type = $car_db,ctime = $createtime, ttk_qty = $ttk_qty, ttk_uom = $ttk_car_db,sgst='".$SGST."',cgst='".$CGST."',igst='".$IGST."' $addval ";
                          ...
                          //      print $updtquery;
                            ...
                            $mid = "";
                             ...
                             //    echo "PO ean is: $EAN<br>";
                               ...
                               $itemname_db = $db->safe($itemname_po);
                               $qpoitem="INSERT INTO it_po_items set po_id = $poid,po_itemname=$itemname_db,dealer_item_id =  $insid $eanaddClause, mrp = $MRP,qty = $Qty, tot_qty = $totQty ,pack_type = $car_db,ctime = $createtime, ttk_qty = $ttk_qty, ttk_uom = $ttk_car_db sgst='".$SGST."',cgst='".$CGST."',igst='".$IGST."' $addval ";
                               echo"<br>insert to poitems=$qpoitem<br>"; 
                               $poitemid=$db->execInsert($qpoitem);   
                             //  exit;
                               $po_amt=$po_amt+$Amount;
                        ...
                        //    print"<br>itemcnt=$itmcnt";
                          ...
                          //      print "<br>IN ) CNT <br>";
                                //chk if all the items are un defined weikfield item
                                  echo $uqry = "select count(*) as cnt from it_po_items p , it_dealer_items d where p.dealer_item_id = d.id and d.is_vlcc = 0 and po_id=$poid";
                                 // exit;
