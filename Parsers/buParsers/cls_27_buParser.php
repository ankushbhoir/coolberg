...
class cls_27_buParser{
    ...
    public function process($file_text){
        ...
        for ($lineno = 0; $lineno < $numlines; $lineno++) {
            ...
            if(preg_match('/Delivery\s+to\s*:/',$line,$matches)){                
         ...
         for ($lineno = $start_line_no ; $lineno < $numlines; $lineno++) {
            $line =  $lines[$lineno];                           
            if(preg_match('/Single\s+Invoice\s+for\s+Multiple\s+PO/',$line,$matches)){
                $end_line_no = $lineno-3;
                break;
            }else  if(preg_match('/Phone\s*:\s*Fax\s*:/',$line,$matches)){
