...
class cls_54_buParser{
    ...
    public function process($file_text){
        ...
        for ($lineno = 0; $lineno < $numlines; $lineno++) {
            ...
            if(preg_match('/(Ship\s*To).*/',$line,$matches) || preg_match('/(Ship\/Bill\s*To).*/',$line,$matches)){
            ...
            if(preg_match('/(Phone)/',$line,$matches)){
...
//                 $line = trim(str_replace("Ship To","",$line)); 
//           }
        ...
        $sobj = $db->fetchObject($query);
         ...
         if(isset($sobj)){
            ...
            $db->closeConnection();
//            print "<br>CALL NEXT 9_1 BU PARSER <br>";
//            $clsname = "cls_9_1_buParser";
