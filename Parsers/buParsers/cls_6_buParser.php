...
class cls_6_buParser{
    ...
    public function process($file_text){
        ...
        for ($lineno = 0; $lineno < $numlines; $lineno++) {
            ...
            if(preg_match('/(GST\s*No).*/',$line,$matches) || preg_match('/(CST\s*No).*/',$line,$matches)){
...
//            }
           $line = $lines[$i]; 
             ...
             if(trim($line)==""){ continue; }                        
             $result = array();
             if(preg_match($regex,$line,$result)||preg_match($regex11,$line,$result)){
                 print "<br>result=";
                 print_r($result);
        ...
        $shipping_address=str_replace("(","", $shipping_address1);
        $shipping_address=str_replace(")","", $shipping_address1); 
        ...
        print "<br> QUERY: $query <br>";
         ...
         if(isset($sobj)){
          ...
          $db->closeConnection();
//            print "<br>CALL NEXT 6_1 BU PARSER <br>";
//            $clsname = "cls_6_1_buParser";
