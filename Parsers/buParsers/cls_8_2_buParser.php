...
//            print "<br> Line $lineno: $line";
        ...
        for ($lineno =  $start_line_no ; $lineno < $numlines; $lineno++) {
            $line = trim($lines[$lineno]);
            print "<br>Start Line Inside Endline loop : $start_line_no<br>";
            if(preg_match('/(Spencer).*/',$line,$matches)){
                $end_line_no = $lineno-1;
                $line = trim($lines[$end_line_no]);
                 if(preg_match('/(Page).*/',$line,$matches)){
...
//             print "<br><br> LINE $i: $line ";
        ...
        $sobj = $db->fetchObject($query);
         ...
         if(isset($sobj)){
            ...
            $db->closeConnection();
            print "<br>CALL NEXT 8_3_BU PARSER <br>";
            $clsname = "cls_8_3_buParser";
