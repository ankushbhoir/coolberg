...
//                     echo "Format: ".$ini->Header->Fields[19]->Name."<br>";
...
//                             print_r($mtch13);                             
                     ...
                     if($master_dealer_id==26){    // Vishal Mega mart                      
...
//                         echo "inside snnr";
		  ...
		  if($master_dealer_id==49){  //V mart
			   ...
			   if($master_dealer_id==32){  //Vijetha
                   ...
                   /*   if($master_dealer_id==2){    // Future retail    
                          echo "***************************FR case***************************<br>";
                          print_r($matches);
                          $i++;
                          $flagP=0;
                          $regex = $ini->Items->Regex[1];
                          if(preg_match($regex,$linetext,$getArtNo)){
                              $matches['ArticleNo'] = trim($getArtNo['ArticleNo']);
                          }
                          if(isset($matches['ArticleNo']) && isset($matches['EAN'])){
                             $i++;                                                          
                             $regex = $ini->Items->Regex[0];
                             for($k=$i;$k<$lastItemRow;$k++){
                                 $linetext = trim($rows[$k]);
                                // if(preg_match('/Description\s+of/', $linetext)==1){
                                   //  $flagP = 1;
                                    // if(preg_match('/Description\s+of/', $linetext)==1){
