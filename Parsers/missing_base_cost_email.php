...
$missean=array();
        ...
        echo "no missing EAN";
          ...
          while ($obj_res = $result_id->fetch_object()) {
               ...
               $po_array=array();
 $subject = "Base Cost Validation Error";
        $body = "<br>Hi<br>";
        $body .= "<p>Base cost is coming Wrong in  Purchase Order  </p>";
        $body .= '
<html>
<head>
  <style>
...
<body>
  <p>Following Missing EAN details</p>
  <table >
    <tr>
      <th>PO No.</th><th>EAN</th><th>PO MRP</th><th>Calulated Rate</th><th>Chain Name</th>
    </tr>';
            ...
            echo $get_from_email="select pd.id,pd.from_email,pd.new_filename,pd.fullpath,p.invoice_no,p.filename from it_po p,it_po_details pd where p.invoice_no='".$obj_res->invoice_no."' and p.filename=pd.new_filename and p.ctime like '".date('Y-m-d')."%' and pd.sent=0;";
               $result_email = $db->getConnection()->query($get_from_email);
                 ...
                 while ($obj_email = $result_email->fetch_object()) {
                  ...
                  $get_info="select * from it_missing_base_price where created_at like '".date('d-m-Y')."%' and sent=0 and po_no='".$obj_res->invoice_no."'";
      ...
      $db->execUpdate("update it_missing_base_price set sent=1 where id=$obj->id");
         
   $body.= "<tr>
      <td>$obj->po_no</td><td>$obj->ean</td><td>$obj->cal_rate</td><td>$obj->porate</td><td>$obj->chain_name</td>
    </tr>";
