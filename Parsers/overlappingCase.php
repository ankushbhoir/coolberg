...
class overlappingCase{
    ...
    public function fetchItemsOVC($startRow,$lastItemRow,$rows,$ini){
       print "<br>START ROW : $startRow :: Last Row: $lastItemRow<br> ROWS => "; 
//       print_r($rows);
       print "<br>Overlapping INI below: <br>";
       print_r($ini);
       $all_items=array();  
       $item = array();
       for ($i=$startRow; $i<$lastItemRow; $i++) { 
           ...
           print"regex:$regular_regex<br>line:$linedata<br>";
           $matches = array();
           if(preg_match($regular_regex, $linedata,$matches)){
               print "<br>Items<br>";
//               print_r($matches);
               $valid = validateitemsOVC($matches);
               if(($valid)==1){
                   ...
                   $fetch_item_descrp = checkEANnoOVC($EAN);
                   ...
                   if(trim($fetch_item_descrp)==1){
                       //step 1 : fetch itm if not found wth regular regex
                        ...
                        if(trim($Itemname)==""){
                            ...
                            $itmdesp = fetchItemDespOVC($optional_regex,$previous_line);
                  ...
                  if(trim($Amount)==""){
                      ...
                      $amt = fetchAmountOVC($amt_regex,$next_line);
                      $matches['Amount'] = doubleval($amt); 
                      
                      $amt1=$matches['Amount'];
                      $rate=doubleval($matches['Rate']);
                      $qty=doubleval($matches['Qty']);
                      
                      $amt_cal= $rate*$qty;
                      
                      print"<br> CAL_AMT:$amt_cal<br> fetch_AMT:$amt1<br>";
                      
                      if(round(trim($amt1)) != round(trim($amt_cal))){
                          $next_lineno -=1;
                          $next_line=$rows[$next_lineno];
                          print"<br>linetext:$rows[$next_lineno]<br>";
                          $amt = fetchAmountOVC($amt_regex,$next_line);
                          $matches['Amount'] = doubleval($amt); 
                      }
                  }
...
function validateitemsOVC($item_arr){
        $item = array();
//        $fields=array();
...
//                $fields['CAR'] = $CAR;
    ...
    function fetchItemDespOVC($optional_regex,$previous_line){
      ...
      function fetchAmountOVC($amt_regex,$next_line){
