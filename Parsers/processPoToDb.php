...
//            }
            ...
            $header = getHeader($rows, $ini, $shipping_address, $master_dealer_id);  // call to getHeader()
            ...
            print"<br>no of items in PO=$noofitems<br> :$iniid ";
        ...
        //                print"<br> set vat & rate =0 for wallmart<br>";
          ...
          //                      print"VAT is null set to 0";
            ...
            //                    print"<br>TAX :".$VAT."<=".DEF_TAX;
              ...
              //                  print"<br>invalid TAX percent".$VAT."<br>";
                  ...
                  //              print_r($item);
                    ...
                    $cal_amt=$item['Qty']*$item['Rate'];
                        ...
                        if($master_dealer_id == 14){                            // only for metro
                          // if(!( trim($item['Qty']) > 0 && trim($item['MRP']) > 0 && trim($item['MRP']) <= 30000 && trim($item['Rate']) > 0 && trim($item['Rate']) <= 2000)){
                          //      print" in Qty,MRP or Rate out of range check<br>".trim($item['MRP']);
                          //       //$ret =  POStatus::STATUS_ISSUE_AT_PROCESSING."::-1";
...
//                           return $resp;
                           ...
                           $resp = printItemsArticleNoBased($header,$items,$invtext,$iniid,$newPOpdf,$shipping_address,$shipping_id,$rows); //ini_id //call to printItems() in SaveToDB_ArticleNoBased.php
...
//                      return $resp;
              ...
              function getHeader($rows, $ini, $shipping_address, $master_dealer_id) {
 //   print_r($rows);
 //    print"<br><br>";
 // print_r($ini);
 //   print"<br><br>";
...
//print_r($fields);
     ...
     //print_r($fields);
                foreach ($fields as $field) {
                  if(isset($field->value) && $field->value == 'ABRL SUPER'){
                    $is_ABRL_super = 1;
                  }
...
echo $value = trim(substr($value,$field->start,$field->length));  //print"val---$value";
} else {
...
$header[$field->Name] = $value;                  
         ...
         echo  'working in 2nd else if llop';
// print_r("asfdgf");
// exit;
               print_r("\n");
               //print_r($field->row);
  ...
  if(isset($field->stopIdentifierRegex) && trim($field->stopIdentifierRegex) != ""){
               //nt "<br>LRI: ".$field->stopIdentifierRegex."<br>";
    // echo "dhantadan";
    // exit;
    ...
    $vaddrlastrow = fetchAddressLastRow($st_row,$field->stopIdentifierRegex,$rows,$field->Regex[0],$jumpUp);
               //print_r($vaddrlastrow);
                //echo "<br>";
               // echo $vaddrlastrow;


        // echo "asdsd".$st_row = $st_row - 1;
        // echo "vaddrlastrow".$vaddrlastrow;
    ...
    for($i=$st_row;$i<$vaddrlastrow;$i++){
     ...
     if(preg_match($field->Regex[0],$rows[$i],$matches)){
                   ...
                   // exit;
                    ...
                    print_r($vaddr);
                    print"<br> ******mm*****";
                    if($field->Name=='VendorName' && $is_ABRL_super){
                     preg_match("/(?'vname'\\s.{30})/", $vaddr,$vandorname);
                 // echo "adsadasddddddd";
                 // echo "<br>";
                 // echo $vaddr;
                 // echo '<pre>';
               ...
               // print"<br>address=$vaddr<br>";
                 ...
                 {
          //  print "<br> Regex: aniket".$field->Regex." <br>";
           // echo "next:asdasldasdljkasd";
         ...
         //  print_r($field->row);
                   ...
                   echo $rows[$i];
                    ...
                    echo "<br>";
                     ...
                     print "<br> Regexhead: ".$field->Regex[$i]."<br>";
           ...
           // echo "LineXXX: ".$rows."<br>";
           // exit;
            for($i=0;$i<count($rows);$i++){
                // echo "$i Line: $rows[$i]<br>";
               //  echo $field->PregMatch[0];
               //  echo "<br>";
...
// written by mayur for Dmart to get vendors address pincode as vendor code

// if($master_dealer_id == 54){
//   $dealerPincode = substr(trim($shipping_address), -6);
//   // echo $dealerPincode;
//   if(is_numeric($dealerPincode)){
//     $header->DealerCode = $dealerPincode;
//   }
// }
if($master_dealer_id==8){
  // echo "adasdasd".$header->DealerCode;
  // exit;
 if($header->DealerCode==''){
  $str=json_encode($rows);
  preg_match('/Delivery\s+Site:\s+(\S+)/', $str, $output_array);
  //print_r($output_array);
 // echo $output_array[1];
  $header->DealerCode = $output_array[1];
  //exit;
}
}
// print_r($ini);
...
function getItems($rows, $ini,$master_dealer_id) {  
  $startRow = 0;
    //echo "asdasdd".$ini->Items->StartRow;
   // exit;
   ...
   echo $ini->Items->RowsPerItem;
    ...
    if(trim($ini->Items->RowsPerItem)==-1){ // appay all regerx to all rows if itemline spencer/Reliance
      ...
      $items = $PER->fetchItems($startRow,$lastItemRow,$rows,$ini,$master_dealer_id);
       print"<br>returndss array items=<br> ";
      ;
      //exit;
    }
...
//        print_r($items);
     ...
     for ($i=$startRow; $i<$lastItemRow; $i++) {
               echo "Line will be: $rows[$i]";
               echo "<br>";
               echo $ini->Items->Regex;
...
function fetchAddressLastRow($startRow,$stopIdentifierRegex,$rows,$regex,$jumpUp){
 ...
 for($i=$startRow;$i<=$eof;$i++){
  ...
  if(preg_match($regex,$rows[$i],$matches)){
            ...
            if(isset($matches["addr"])){
              $linedata = trim($matches["addr"]);
            }
          }
