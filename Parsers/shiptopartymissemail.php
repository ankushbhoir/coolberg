...
$missean=array();
      ...
      echo  $main_query="select id,invoice_no from it_po where ctime like '".date('Y-m-d')."%'  and status =6";
        ...
        echo "no missing site";
          ...
          while ($obj_res = $result_id->fetch_object()) {
               ...
               $po_array=array();
 $subject = "Missing SITE";
        $body = "<br>Hi<br>";
        $body .= "<p>Ship to Party Mapping Master Data is Missing for Purchase Order  </p>";
        $body .= '
<html>
<head>
  <style>
...
<body>
  <p>Following Missing SITE details</p>
  <table >
    <tr>
      <th>PO No.</th><th>SITE</th><th>Chain Name</th>
    </tr>';
            ...
            echo $get_from_email="select pd.id,pd.from_email,pd.new_filename,pd.fullpath,p.invoice_no,p.filename from it_po p,it_po_details pd where p.invoice_no='".$obj_res->invoice_no."' and p.filename=pd.new_filename and p.ctime like '".date('Y-m-d')."%' and pd.sent=0;";
               $result_email = $db->getConnection()->query($get_from_email);
                 ...
                 while ($obj_email = $result_email->fetch_object()) {
                  ...
                  $get_info="select * from it_missing_site where created_at like '".date('d-m-Y')."%' and sent=0 and po_no='".$obj_res->invoice_no."'";
      ...
      $db->execUpdate("update it_missing_site set sent=1 where id=$obj->id");
         
   $body.= "<tr>
      <td>$obj->po_no</td><td>$obj->site</td><td>$obj->chain_name</td>
    </tr>";
...
$emailHelper = new EmailHelper();
        ...
        $query_get_sku_emails = "select emails from it_email_action where id=1";
