...
// Create a first sheets
    ...
    $srno=1;
    $db = new DBConn();
    $mnth_srt = date('Y-m-01');
    $mnth_srt_dt = $mnth_srt." 00:00:00 ";
    $mnth_srt_dt_db = $db->safe(trim($mnth_srt_dt));
    $today_dt = date('Y-m-d');
...
//    $st_dt_db = $db->safe(trim($st_dt));
    ...
    $msg='PO Processed Report from '.$mnth_srt .' to '. $today_dt;
    ...
    $query = "select m.name as name, count(*) as cnt from it_receivedpos r, it_master_dealers m  where r.createtime between $mnth_srt_dt_db and $ed_dt_db and r.master_dealer_id= m.id group by master_dealer_id ";
    ...
    $rowCnt = 5;
    $query1 ="select p.master_dealer_id as mid, m.name as chain_name,p.status,p.is_Current_status, count(*) as cnt from it_process_status p, it_master_dealers m where p.master_dealer_id = m.id and p.createtime between $mnth_srt_dt_db and $ed_dt_db and is_current_status=1 group by p.master_dealer_id, status"; 
             //"select p.master_dealer_id as mid, m.name as chain_name, status, count(*) as cnt from it_process_status p, it_master_dealers m  where p.master_dealer_id = m.id and p. createtime between '2016-12-01 00:00:00' and '2016-12-16 23:59:59' and p.is_current_status = 1 group by p.master_dealer_id , p.status";
...
Missing EAN = PO is marked as missing EAN when there is no record found in our database for the given EAN number in the PO.';
    ...
    $nowtime = date('Y-m-d');
    $name = "statusReport_MTD_" . $nowtime;
    $Ext = ".xls";
