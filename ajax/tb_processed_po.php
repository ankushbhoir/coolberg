...
// print_r($chainId);
// exit();
$aColumns = array('checkBox','invoice_no','so_number','sap_message', 'chain_name', 'ctime','action');
$sColumns = array('p.invoice_no', 'md.name', 'p.so_number','p.sap_message','p.ctime');
...
$sWhere .= "p.status  in (11,21) and p.master_dealer_id = md.id $addWhere $addChain";
...
$sQuery = "
    select SQL_CALC_FOUND_ROWS p.id, p.filename,p.ctime,md.displayname as chain_name,p.invoice_no,p.so_number,p.sap_message from it_po p, it_master_dealers md 
        $sWhere
    $sOrder
    $sLimit
";
...
foreach ($objs as $obj) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($aColumns[$i] == "checkBox") {
        ...
        }else if ($aColumns[$i] == "so_number") {
            $row[] = clean($obj->so_number);
        }else if ($aColumns[$i] == "sap_message") {
            $row[] = clean($obj->sap_message);
            ...
            if(isset($obj_details->fullpath) && $obj_details->fullpath != null){
                if (strpos($obj_details->fullpath, 'ttkstg') !== false) {
                     $replace_path='/home/ttkstg/public_html';
                    }
                    else{
                          $replace_path='/home/dt/public_html';
                    }
                
                $temp_path=str_replace($replace_path, '', $obj_details->fullpath);
                $finalpath= str_replace('newPOs', 'missingArticleNo', $temp_path);
