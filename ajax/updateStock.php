...
try {
    ...
    if (count($error) == 0) {
        ...
        foreach ($qryObj as $obj) {
            ...
            $saId = $dbl->insertStockAdjustmentHeader($crid, $prodId, $userid);
            if ($saId > 0) {
                $id = $dbl->insertStockAdjustmentDetails($crid, $prodId, $name, $desc1, $desc2, $thickness, $length, $hsncode, $oldqty, $addQty, $saId);
                if ($id > 0) {
                    ...
                    $resp = array(
                        "error" => "1",
                        "msg" => "Enable to load items datails into database."
                    );
                    echo json_encode($resp);
                }
            } else {
                $resp = array(
                    "error" => "1",
                    "msg" => "Enable to load header details into database."
                );
                echo json_encode($resp);
            }
        }
