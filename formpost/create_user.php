...
//            print_r($userId);
            if (isset($obj1) && !empty($obj1)) {
                ...
                $last_inserted_id = $dbl->addUser($userId, $username, $password, $name, $email, $phone, $usertype, $crid, $hashValue, $dcid);
                if (trim($last_inserted_id) <= 0) {
                    $error['insert_fail'] = "New user is not created. Try to create it again.";
                } else {
                    $result = $dbl->assignRoleIdToUser($last_inserted_id,$usertype,$userId);
                    if($result <= 0){
                        $error['role_assign_fail'] = "New created. But user role is not assigned. Please assign role from master.";
                    }else{
