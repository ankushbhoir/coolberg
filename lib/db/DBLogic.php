...
//            $addquery .= " ,password = md5('" . $password_db . "')";
            $password_hash = password_hash($password_db, PASSWORD_BCRYPT);
            $addquery .= " ,password = '" . $password_hash . "'";
        }
        ...
        $query = "insert into it_users set created_by = $userid $addquery, password_updated_at = now()";
        echo $query;
        $po_id = $this->db->execInsert($query);
        $this->db->closeConnection();
        if (isset($po_id) && $po_id > 0) {
...
//    $query = "update it_users set password = md5($pwd1_db) , updatetime = now() where username = $username_db";
    $query = "update it_users set password = '$password_hash' , updatetime = now() where username = $username_db";
    $this->db->execUpdate($query);
    $this->db->closeConnection();
}
...
function getStockTransferItems($transferid) {
    ...
    function getBatchCodeByProductid($productid, $fromlocid, $fromloctype,$challanid = FALSE) {
        ...
        $batchcodes="";
        if(isset($challanid)){
            $bquery="select batchcode from st_challan_items where stc_id = $challanid";
            $objs = $this->db->fetchObjectArray($bquery);
            $batchcode="";
            if(isset($objs)){
                foreach($objs as $obj){
                    $batchcodes .= $obj->batchcode.",";
                }
                $batchcodes = rtrim($batchcodes,',');
                if($batchcodes !=""){
                    $challanidcls= " and sc.batchcode not in ($batchcodes)";                 
                }
            }           
        }
        if ($fromloctype == LocationType::DC) {
        ...
        $query = "select sc.id,sc.batchcode,sc.qty,gl.length,gl.no_of_pieces from it_stockcurr sc,it_grnitems gl where sc.batchcode = gl.batchcode and sc.prodid =$productid and sc.qty > 0 $addqry $challanidcls order by sc.batchcode";
...
//    function getCRStockSummery($crid, $uploaddate) {
 ...
 //               . "round(round(s.qty,4) * pr.price,2) as value,s.createtime from it_products p,it_stockcurr s,it_categories c,it_product_price pr,it_grnitems gl "
 //               . "where p.id = s.prodid and s.prodid = pr.product_id and s.batchcode = gl.batchcode and  p.ctg_id = c.id and "
 //               . "date(pr.applicable_date) = '".$uploaddate."' and pr.crid = $crid and s.crid = $crid order by s.id desc";
//        echo $query;
//        $obj_stockdetail = $this->db->fetchObjectArray($query);
//        $this->db->closeConnection();
//        if (isset($obj_stockdetail) && $obj_stockdetail != NULL) {
...
function insertCreditNote($invid, $invdate, $invoiceno, $customer_id, $cname, $cphone, $cndate, $cnnum, $cnstatus, $userid, $discount){
 ...
 $query = "insert into it_creditnote set cnno = $cnnum_db, crid = $crid, cndate = $cndate_db, invoiceid = $invid, invoice_no = $invno_db, invoice_date = $invdate_db,"
...
function insertIntoImprestDetails($amount, $description, $voucher_no, $userid, $crid, $prevBal, $newBal, $impreason, $impledger){

    $this->db = new DBConn();
    $paymentdiaryqry = "insert into it_imprest_details set amount = $amount ,prev_bal = $prevBal, curr_bal= $newBal, reason = $impreason, voucher_no = '$voucher_no' , description = '$description' , by_user = $userid, crid = $crid, ctime = now(), ledger_id = $impledger ";
        //echo $paymentdiaryqry;
//        error_log("\n payments_diary    : $paymentdiaryqry \n",3,"../tmp.txt");
    $insertId = $this->db->execInsert($paymentdiaryqry);
    return $insertId;
}
...
function addUser($userId, $username, $password, $name, $email, $phone, $usertype, $crid, $hashValue, $dcid){
    $this->db = new DBConn();
    $addQuery = "";
    if($crid != ""){
...
function updateUser($userId, $name, $email, $phone, $username, $password, $inactive, $uid){
    ...
    $email = $this->db->safe($email);
    $phone = $this->db->safe($phone);      
    $query = "update it_users set username = $username, password = $password, name = $name, email = $email, phoneno = $phone, updated_by = $userId , inactive = $inactive, updatetime = now(), password_updated_at = now() where id = $uid";
    $insertId = $this->db->execUpdate($query);
    return $insertId;
}
...
function getChallanItems($challanid) {
    $this->db = new DBConn();
    $query = "select p.id as prodid, p.name as prod, p.desc1 as desc_1, p.desc2 as desc_2, p.thickness as thickness, p.kg_per_pc , stc.*,sp.name as spec, p.hsncode as hsncode, st.qty as req_qty,  numberpcs as pcs from st_challan_items stc, it_products p, it_specifications sp, it_stock_transfer_items st where p.id = stc.prodid and sp.id = p.spec_id  and st.id = stc.stitem_id and stc_id = $challanid";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if (isset($obj) && $obj != NULL) {
        return $obj;
...
function insertStockAdjustmentDetails($crid,$prodId,$name,$desc1,$desc2,$thickness,$length,$hsncode,$oldqty,$addQty,$saId){
    $this->db = new DBConn();
    $query = "insert into stockadjustmentItemDetails set crid = $crid,prodid=$prodId,name='".$name."',desc1='".$desc1."',desc2='".$desc2."',thickness = '".$thickness."',length = '".$length."',hsncode = $hsncode, oldStock = $oldqty, addedstock = $addQty, saID = $saId";
    $id = $this->db->execInsert($query);
    $this->db->closeConnection();
    return $id;
}
...
function approveStockAdjustment( $id, $userid) {
    $this->db = new DBConn();
    $query = "update stockadjustmentHeader set isApproved = 1, approvedBy = $userid, approvedate = now() where id = $id";
    $id = $this->db->execUpdate($query);
    $this->db->closeConnection();
    return $id;
}

function disapproveStockAdjustment($id,$userid) {
    $this->db = new DBConn();
    $query = "update stockadjustmentHeader set isApproved = 2, disapprovedBy = $userid, disapprovedate = now() where id = $id";
    $id = $this->db->execUpdate($query);
    $this->db->closeConnection();
    return $id;
}
...
function fetchProductPriceByProdIdCrid($prodid, $crid) {
 $this->db = new DBConn();
 $query = "select p.kg_per_pc,pr.* from it_product_price pr, it_products p where p.id = pr.product_id and pr.product_id = $prodid and pr.is_approved = true and date(pr.applicable_date) = curdate() and crid = $crid and pr.applicable_date = (select max(applicable_date) from it_product_price where crid = $crid and is_approved = true)";
//        $query = "select p.kg_per_pc,pr.* from it_product_price pr, it_products p where p.id = pr.product_id and pr.product_id = $prodid and pr.is_approved = true and date(pr.applicable_date) = curdate() and pr.applicable_date = (select max(applicable_date) from it_product_price)";
//        print_r($query);
...
//        $this->db = new DBConn();
 ...
 //               . "round(round(sum(s.qty),4) * pr.price,2) as value,s.createtime from it_products p,it_stockcurr s,it_categories c,it_product_price pr, it_grnitems gi where "
   //             . "p.id = s.prodid and s.prodid = pr.product_id and s.batchcode = gi.batchcode and  p.ctg_id = c.id and pr.applicable_date ='".$uploaddate."' and s.crid = $crid and pr.crid = $crid "
...
function insertCR($dispname,$rfcname,$cntper,$address,$email,$phone,$gstno,$panno,$state,$set,$userid) {
    $this->db = new DBConn();
    $cr_id = 0;
    $crCode = "cr270001";
    $crCode_db = $this->db->safe($crCode);
    ...
    . " address = $address_db, emailaddress = $email_db, phoneno = $phone_db, gstno = $gstno_db, panno = $panno_db, cust_name = 'SC$dispname',"
...
function getCRCnSummeryWithDtrange($crid,$FromDate,$ToDate) {
   $this->db = new DBConn();
   $crdetails = $this->getCRInfoById($crid);
   $crcode = $crdetails->crcode;
   $query = "select c.cnno, c.cndate,c.invoice_no,c.invoice_date,c.cname,c.cphone,p.name,p.desc1,p.desc2,p.thickness,p.hsncode,cl.batchcode,gl.length,cl.qty,cl.mrp,cl.invcuttingcharges,cl.rate,cl.taxable,cl.cgstval,cl.sgstval,cl.total,cl.createtime,c2.gstno, cl.cgstpct, cl.sgstpct "
   . "from it_creditnote c left outer join it_customers c2 on  c.customerid= c2.id,it_creditnote_items cl, it_products p,it_grnitems gl where c.id = cl.cnid and p.id = cl.product_id and gl.batchcode = cl.batchcode  and c.crid = $crid and c.status = 1 and c.cndate >='".$FromDate."' and c.cndate <='".$ToDate."' order by c.cndate desc";
        //echo $query;
   $obj_salesdetail = $this->db->fetchObjectArray($query);
   $this->db->closeConnection();
   if (isset($obj_salesdetail) && $obj_salesdetail != NULL) {
...
function getAggCRCnSummeryWithDtrange($crid,$FromDate,$ToDate) {
  $this->db = new DBConn();
  $crdetails = $this->getCRInfoById($crid);
  $crcode = $crdetails->crcode;
  $query="select
  ...
  c.cphone,
  sum(cl.qty) as qty,
  sum(cl.rate) as rate,(case when c.cndate > '2019-01-10' then sum(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)) else sum(cl.taxable) end) as taxable,
  (case when c.cndate > '2019-01-10' then sum(round((cgstpct/100) *(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)),2)) else sum(cl.cgstval) end) as cgst_amt,
  (case when c.cndate > '2019-01-10' then sum(round((sgstpct/100) *(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)),2)) else sum(cl.sgstval) end) as sgst_amt ,
  (case when c.cndate > '2019-01-10' then ((sum(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)))
  +(sum(round((cgstpct/100) *(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)),2)))
  +(sum(round((sgstpct/100) *(round(CAST(qty AS DECIMAL(9,4))* cast(rate AS DECIMAl(9,2)),2)),2)))
  ) else sum(cl.total) end) as total,
  cl.createtime,c2.gstno"
  . " from it_creditnote c left outer join it_customers c2 on  c.customerid= c2.id, it_creditnote_items cl,it_products p where c.id = cl.cnid and p.id = cl.product_id and c.status = 1 and c.cndate >='".$FromDate."' and c.cndate <='".$ToDate."' and c.crid = $crid group by c.cnno order by c.cndate";
//        print_r($query);  
  $obj_salesdetail = $this->db->fetchObjectArray($query);
  $this->db->closeConnection();
  if (isset($obj_salesdetail) && $obj_salesdetail != NULL) {
...
function getAggCRSalesSummeryWithDtrange($crid,$FromDate,$ToDate) {
  ...
  $crcode = $crdetails->crcode;        
        //$query = "select c.invoice_no,c.saledate,c.cname,c.cphone,sum(cl.qty) as qty,sum(cl.rate) as rate,
        //          (case when c.saledate > '2019-01-10' then sum(round(CAST(cl.rate AS DECIMAL(9,2) ) * CAST(cl.qty AS DECIMAL(9,4)),2)) else sum(cl.taxable) end) as taxable,
        //          (case when c.saledate > '2019-01-10' then sum(round(CAST(cl.cgst_amt AS DECIMAL(9,2)) * CAST(cl.qty AS DECIMAL(9,4)),2)) else sum(cl.cgst_amt) end) as cgst_amt,
        //          (case when c.saledate > '2019-01-10' then sum(round(CAST(cl.sgst_amt AS DECIMAL(9,2)) * CAST(cl.qty AS DECIMAL(9,4)),2)) else sum(cl.sgst_amt) end) as sgst_amt
...
//        $this->db = new DBConn();
//        $query = "select c.name as ctg,p.name,p.desc1,p.desc2,p.thickness,p.hsncode,p.stdlength,s.batchcode,gl.length ,round(s.qty,4) as qty,pr.price as price,"
 //               . "round(round(s.qty,4) * pr.price,2) as value,s.stock_date as createtime from it_products p,it_closing_stock s,it_categories c,it_product_price pr,it_grnitems gl "
 //               . "where p.id = s.prodid and s.prodid = pr.product_id and s.batchcode = gl.batchcode and  p.ctg_id = c.id and "
 //               . "date(pr.applicable_date) = '".$date."' and pr.crid = $crid and s.crid = $crid and stock_date = '$date' order by s.id desc";
//        echo $query;
//        $obj_stockdetail = $this->db->fetchObjectArray($query);
//        $this->db->closeConnection();
//        if (isset($obj_stockdetail) && $obj_stockdetail != NULL) {
...
//        $this->db = new DBConn();
//        $query = "select c.name as ctg,p.name,p.desc1,p.desc2,p.thickness,p.hsncode, gi.length as stdlength,round(sum(s.qty),4) as qty,pr.price as price,"
 //               . "round(round(sum(s.qty),4) * pr.price,2) as value,s.stock_date as createtime from it_products p,it_closing_stock s,it_categories c,it_product_price pr, it_grnitems gi where "
 //               . "p.id = s.prodid and s.prodid = pr.product_id and s.batchcode = gi.batchcode and  p.ctg_id = c.id and date(pr.applicable_date) ='".$closeDate."' and s.crid = $crid and pr.crid = $crid and s.stock_date = '$closeDate' "
 //               . "group by ctg,p.name,p.desc1,p.desc2,p.thickness,p.hsncode, gi.length order by p.name";
//    echo $query;
 //       $obj_stockdetail = $this->db->fetchObjectArray($query);
 //       $this->db->closeConnection();
...
function getImprestExcelReport($crid,$fromDate, $toDate){
    $this->db = new DBConn();
    $status = StockTransferStatus::Completed;         
    $query = "SELECT r.dispname,i.prev_bal,i.amount,i.curr_bal,i.voucher_no,i.description,i.reason,u.name,i.ctime,l.ledger FROM it_imprest_details i, it_rfc_master r,it_users u, it_imprest_ledger l where u.id= i.by_user and r.id = i.crid and i.ctime > '$fromDate' and i.ctime < '$toDate' and i.crid = $crid and l.id = i.ledger_id";
...
function getLatestProductPrice($crid, $prodId, $applicableDate){
    $this->db = new DBConn();
    $selectstockcurrent = "select price From it_product_price p where p.applicable_date = (select max(pp.applicable_date ) "
...
function getImprestExcelReportDC($dcid,$fromDate, $toDate){
    $this->db = new DBConn();
    $status = StockTransferStatus::Completed;
    $query = "SELECT d.dc_name,i.prev_bal,i.amount,i.curr_bal,i.voucher_no,i.description,i.reason,u.name,i.ctime, l.ledger FROM it_imprest_details i, it_dc_master d,it_users u, it_imprest_ledger l where u.id= i.by_user and d.id = i.dcid and i.ctime > '$fromDate' and i.ctime < '$toDate' and i.dcid = $dcid and l.id = i.ledger_id order by i.ctime desc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    return $obj;
}

function insertStockAdjustmentDetailsDC($prodId,$name,$desc1,$desc2,$thickness,$length,$hsncode,$oldqty,$addQty,$dcid,$saID){
    $this->db = new DBConn();

    $query = "insert into stockadjustmentItemDetails set prodid=$prodId,name='".$name."',desc1='".$desc1."',desc2='".$desc2."',thickness = '".$thickness."',length = '".$length."',hsncode = $hsncode, oldStock = $oldqty, addedstock = $addQty, dcid = $dcid, saID = $saID";
...
function insertStockAdjustmentHeaderDC($prodId,$userid,$dcid){
    $this->db = new DBConn();
    $query = "insert into stockadjustmentHeader set prodid=$prodId,requestBy=$userid, dcid = $dcid ";
    $id = $this->db->execInsert($query);
    $this->db->closeConnection();
    return $id;
}

function getDCInfoById($dcid) {
    $this->db = new DBConn();
    $query = "select * from it_dc_master where id = $dcid";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    return $obj;
}

function approveStockAdjustmentDC( $id, $userid) {
    $this->db = new DBConn();
        //$query = "update stockadjustmentHeader set isApproved = 1, approvedBy = $userid, approvedate = now() where id = $id and prodid=$prodid and isApproved=0 and dcid = $dcid and id>0 ";
    $query = "update stockadjustmentHeader set isApproved = 1, approvedBy = $userid, approvedate = now() where id = $id";
       // echo $query;
    $id =$this->db->execUpdate($query);
    $this->db->closeConnection();
...
function getLatestBatchCodeByProdID_DC($prodid,$dcid) {
    $this->db = new DBConn();
    $query = "select *  from it_stockcurr where prodid=$prodid and dcid=$dcid order by createtime desc limit 1";
    $obj_batchcode = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if (isset($obj_batchcode)) {
        return $obj_batchcode;
...
function getImprestByDate($datetime){
    $this->db = new DBConn();
    $out= ImprestReason::Out;
    $datetime_db = $this->db->safe($datetime);
//        $query ="select d.*, l.ledger from it_imprest_details d left join it_imprest_ledger l on d.ledger_id = l.id where d.ctime > $datetime_db and l.Reason = $out order by d.ctime ";
    $query ="select d.*, l.ledger from it_imprest_details d left join it_imprest_ledger l on d.ledger_id = l.id where d.ctime > $datetime_db and d.reason = $out order by d.ctime ";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    return $obj;
}
...
function getImprestledgerByReason($reasontype) {
    $this->db = new DBConn();
    $query = "select l.ledger, l.id  from it_imprest_ledger l where l.Reason = $reasontype ";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if (isset($obj)) {
        return $obj;
...
function getAllRoles(){
    $this->db = new DBConn();
    $query = "select id,name from it_roles  where is_active = 1";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    return $obj;
}
...
function getMaxAvailableDateInClosingStock($FromDate){
    $this->db = new DBConn();
    $query = "select max(stock_date) as stock_date from it_closing_stock where stock_date < '$FromDate'";
    $obj_date = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if (isset($obj_date)) {
        return $obj_date;
    } else {
...
function getOpeningStock($oneDayBefore,$crid,$dcid){
    $this->db = new DBConn();
    if($crid == null){
        $addQuery = "and cs.dcid = $dcid";
    }else{
        $addQuery = "and cs.crid = $crid";
    }
    $query = "select p.id,c.name as category,p.name as product,p.desc1,p.desc2,p.thickness,p.hsncode,gi.length,gi.batchcode, case when cs.qty is null then 0.0000 else round(cs.qty,4) end as qty "
    . "from it_products p, it_grnitems gi left outer join it_closing_stock cs on gi.batchcode = cs.batchcode $addQuery and cs.stock_date = '$oneDayBefore', it_categories c "
    . "where p.id = gi.product_id and c.id = p.ctg_id";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    return $obj;
}
...
function getTotalQtyByReason($crid,$dcid,$batchcode,$from_date,$end_date,$reason){
    $this->db = new DBConn();
    $addLoc = "";
    $addFromDate = "";
    $addEndDate = "";
    if(isset($crid) && $crid != null && $crid != ""){
        $addLoc = "and sd.crid = $crid";
    }else{
        $addLoc = "and sd.dcid = $dcid";
    }

    if(isset($from_date) && $from_date != null && $from_date != ""){
        $addFromDate = "and sd.createtime > "."'"."$from_date 00:00:00"."'";
    }

    if(isset($end_date) && $end_date != null && $end_date != ""){
        $addEndDate = "and sd.createtime < "."'"."$end_date 23:59:59"."'";
    }
    if($reason == StockDiaryReason::Sale || $reason == StockDiaryReason::CRStockpulled || $reason == StockDiaryReason::StockTransferOut){
        $query = "select sum(round(qty/1000,4)) as qty from it_stockdiary sd where sd.reason = $reason and sd.batchcode = '$batchcode' $addLoc $addFromDate $addEndDate";
    }else if($reason == StockDiaryReason::CreditNote || $reason == StockDiaryReason::ChallanOut || $reason == StockDiaryReason::ChallanIn || $reason == StockDiaryReason::StockAdjustment){
        $query = "select sum(round(qty,4)) as qty from it_stockdiary sd where sd.reason = $reason and sd.batchcode = '$batchcode' $addLoc $addFromDate $addEndDate";
    }else if($reason == StockDiaryReason::PurchaseIn){
        $query = "select sum(round(sd.qty/multply,4)) as qty from it_stockdiary sd, it_grnitems gi, it_uom u where u.id = gi.uom_id and gi.batchcode = sd.batchcode and sd.reason = $reason and sd.batchcode = '$batchcode' $addLoc $addFromDate $addEndDate";
    }
//        print_r($query);
    $obj_qty = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if (isset($obj_qty)) {
        return $obj_qty;
    } else {
...
function getAllGRNItems(){
    $this->db = new DBConn();
    $query = "select distinct gi.batchcode , p.id,c.name as category,p.name as product,p.desc1,p.desc2,p.thickness,p.hsncode,gi.length "
    . "from it_products p, it_grnitems gi, it_categories c where p.id = gi.product_id and c.id = p.ctg_id";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    return $obj;
}

function disapproveStockAdjustmentDC($id, $userid) {
	$this->db = new DBConn();
	$query = "update stockadjustmentHeader set isApproved = 2, disapprovedBy = $userid, disapprovedate = now() where id = $id";
	$id = $this->db->execUpdate($query);
	$this->db->closeConnection();
...
function insertNewRole($new_role){
    $this->db = new DBConn();
    $insertqry = "insert into it_roles set name = '$new_role', is_active = 1, ctime = now(), utime = now()";
//        print_r($insertqry);
    $insertId = $this->db->execInsert($insertqry);
    return $insertId;
}

function checkRoleIfExist($role) {
    $this->db = new DBConn();
    $role = trim($role);
    $query = "select id from it_roles where name = '$role'";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if (isset($obj) && $obj != NULL) {
        return $obj;
...
function assignRoleIdToUser($newUserId,$usertype,$userid){
    $this->db = new DBConn();
    $insertqry = "insert into it_user_roles set userid = $newUserId, roleid = $usertype, updated_by = $userid, ctime = now(), utime = now()";
    $insertId = $this->db->execInsert($insertqry);
    return $insertId;
}
...
function getChainList(){
    $this->db = new DBConn();
    $query = "select id,displayname from it_master_dealers where show_code=1 order by name asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getItemCategoryList(){
    $this->db = new DBConn();
    $query = "select id,category from it_category where category != '' and category is not null order by category asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getStateList(){
    $this->db = new DBConn();
    $query = "select id,name from it_regions where zone_id != 0 order by name asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function checkShippingDetails($shiptoparty,$siteindentifier,$site_identifier_type,$customer_name,$distribution_channel,$sales_document_type,$distribution_channel_code){
    $this->db = new DBConn();
    $query = "select id from it_ship_to_party sh where sh.ship_to_party='$shiptoparty' and sh.site='$siteindentifier' and sh.site_identifier_type='$site_identifier_type' and sh.customer_name='$customer_name' and sh.distribution_channel='$distribution_channel' and sh.sales_document_type='$sales_document_type' and sh.distribution_channel_code='$distribution_channel_code'";
    // echo $query;
    // exit;
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
...
function checkShippingAddressInBusinessUnit($dcAddress){
    $this->db = new DBConn();
    $query = "select id from it_business_unit bu where replace(bu.bu_identifier,' ','') = replace('$dcAddress',' ','')";
    $obj = $this->db->fetchObject($query);
    // print_r($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function insertShippingAddress($chain,$iniList,$dcAddress,$dcName,$dcCity,$state,$custCode){
    $this->db = new DBConn();
    $insertqry = "insert into it_shipping_address set shipping_address='$dcAddress',master_dealer_id=$chain,ini_id='$iniList',dc_name='$dcName',dc_address = '$dcAddress',dc_city='$dcCity',customer_code = '$custCode',dc_state='$state',createtime=now()";
       // print_r($insertqry);
    $insertId = $this->db->execInsert($insertqry);
    return $insertId;
}

function getIniList($chain){
    $this->db = new DBConn();
    $query = "SELECT GROUP_CONCAT(id) as iniList FROM it_inis where master_dealer_id = $chain";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getStateName($state){
    $this->db = new DBConn();
    $query = "SELECT name FROM it_regions where id = $state";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function insertBussinessUnit($chain,$storeCode,$dcAddress){
    $this->db = new DBConn();
    $no_spaces = str_replace(" ", "", $dcAddress);
    $insertqry = "insert into it_business_unit set code='$storeCode',master_dealer_id=$chain,bu_address = '$dcAddress',bu_identifier = '$no_spaces',createtime=now()";
    $insertId = $this->db->execInsert($insertqry);
    return $insertId;
}
...
function insertUserRole($utypesel,$user_id,$login_userId){
    $this->db = new DBConn();
    $insertqry = "insert into it_user_roles set userid='$user_id',roleid = '$utypesel',updated_by = '$login_userId' ,ctime=now()";
    $insertId = $this->db->execInsert($insertqry);
    return $insertId;
}

function getshiptopartyDetailsByShippingId($shippingId){
    $this->db = new DBConn();
    $query = "select s.id, s.ship_to_party, m.displayname, s.site,s.site_identifier_type,s.category,s.plant,s.customer_name,s.margin,s.distribution_channel,s.sales_document_type,s.distribution_channel_code from it_ship_to_party s, it_master_dealers m where s.master_dealer_id = m.id and s.id=$shippingId";
//    echo $query;
   $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function updateShippingDetails($shippingid,$shiptoparty,$siteindentifier,$site_identifier_type,$customer_name,$distribution_channel,$sales_document_type,$distribution_channel_code,$old_shiptoparty,$old_siteidentifier,$old_site_identifier_type,$old_customer_name,$old_distribution_channel,$old_sales_document_type,$old_distribution_channel_code) {
    $this->db = new DBConn();

    $addQuery = "";
    if($shiptoparty != $old_shiptoparty){
        $addQuery .= " ship_to_party = '$shiptoparty'";
    }
    if($siteindentifier != $old_siteidentifier){
        $addQuery .= ", site= '$siteindentifier'";
    }
    if($site_identifier_type != $old_site_identifier_type){
        $addQuery .= ", site_identifier_type= '$site_identifier_type'";
    }
//    if($category != $old_category){
//        $addQuery .= ", category = '$category'";
//    }
//    if($plant != $old_plant){
//        $addQuery .= ", plant = '$plant'";
//    }
    if($customer_name != $old_customer_name){
        $addQuery .= ", customer_name = '$customer_name'";
    }
//    if($margin != $old_margin){
//        $addQuery .= ", margin = '$margin'";
//    }
    if($distribution_channel != $old_distribution_channel){
        $addQuery .= ", distribution_channel = '$distribution_channel'";
    }
    if($sales_document_type != $old_sales_document_type){
        $addQuery .= ", sales_document_type = '$sales_document_type'";
    }
    if($distribution_channel_code != $old_distribution_channel_code){
        $addQuery .= ", distribution_channel_code = '$distribution_channel_code'";
    }
  
    $addQuery = ltrim($addQuery, ',');
    $query = "update it_ship_to_party set $addQuery where id = $shippingid";
    // print_r($query);
    // exit;
    $id = $this->db->execUpdate($query);
...
function insertIntoLocationEditDiary($dcAddress,$dcName,$dcCity,$custCode,$shippingId,$old_dcAddress,$old_dcName,$old_custCode,$old_dcCity,$storeCode,$old_storeCode) {
    $this->db = new DBConn();
    $curr_userId = getCurrStoreId();

    $addQuery = "";
    if($dcName != $old_dcName){
        $addQuery .= ", updated_dcname = '$dcName', old_dcname = '$old_dcName'";
    }
    if($dcCity != $old_dcCity){
        $addQuery .= ", Updated_dcCity = '$dcCity', old_dcCity = '$old_dcCity'";
    }
    if($custCode != $old_custCode){
        $addQuery .= ",updated_custcode = '$custCode' , old_custcode = '$old_custCode'";
    }
    if($storeCode != $old_storeCode){
        $addQuery .= ",updated_storecode = '$storeCode', old_storecode = '$old_storeCode'";
    }
    if($dcAddress != $old_dcAddress){
        $addQuery .= ",updated_dcAddress = '$dcAddress', old_dcAddress = '$old_dcAddress'";
    }


    $query = "insert into it_location_edit_diary set shippind_id = '$shippingId', by_user =  $curr_userId, ctime = now() $addQuery";
    // print_r($query);
    $id = $this->db->execInsert($query);
    // print_r($id);
    $this->db->closeConnection();
...
function updateBussinessUnit($bu_id,$storeCode,$dcAddress,$old_storeCode) {
    $this->db = new DBConn();

    $addQuery = "";
    if($storeCode != $old_storeCode){
        $addQuery .= ", code = '$storeCode'";
    }

    $query = "update it_business_unit set bu_address = '$dcAddress' $addQuery where id = $bu_id";
    // print_r($query);
    $id = $this->db->execUpdate($query);
...
function getProductDetailsByDealerItemId($dealerItemId){
    $this->db = new DBConn();
    $query = "select mi.id as itemId, md.name as 'Chain_Name', idt.itemcode as 'Artical_No', mi.itemname as 'Product_Description', mi.mrp as MRP, mi.itemcode as EAN, mi.product_code as 'FG_Code' from it_master_dealers md, it_master_items mi,it_dealer_items idt where idt.master_dealer_id = md.id and idt.master_item_id = mi.id and idt.id =  '$dealerItemId'";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function updateMasterItem($itemId,$prod_desc,$mrp,$ean,$fg_code,$old_prod_desc,$old_mrp,$old_ean,$old_fg_code) {
    $this->db = new DBConn();

    $addQuery = "";
    if($prod_desc != $old_prod_desc){
        $addQuery .= ", itemname = '$prod_desc'";
    }
    if($mrp != $old_mrp){
        $addQuery .= ", mrp = '$mrp'";
    }
    if($ean != $old_ean){
        $addQuery .= ", itemcode = '$ean'";
    }
    if($fg_code != $old_fg_code){
        $addQuery .= ", product_code = '$fg_code'";
    }

    $query = "update it_master_items set updatetime = now() $addQuery where id = $itemId";
    // print_r($query);
    // return;
    $id = $this->db->execUpdate($query);
...
function updateDealerItems($productId,$chain_article,$old_chain_article) {
    $this->db = new DBConn();

    $addQuery = "";
    if($chain_article != $old_chain_article){
        $addQuery .= ", itemcode = '$chain_article'";
    }

    $query = "update it_dealer_items set updatetime = now() $addQuery where id = $productId";
    // print_r($query);
    // return;
    $id = $this->db->execUpdate($query);
...
function insertIntoProductEditDiary($productId,$itemId,$chain_article,$prod_desc,$mrp,$ean,$fg_code,$old_chain_article,$old_prod_desc,$old_mrp,$old_ean,$old_fg_code) {
    $this->db = new DBConn();
    $curr_userId = getCurrStoreId();

    $addQuery = "";
    if($chain_article != $old_chain_article){
        $addQuery .= ", updated_article_code = '$chain_article', old_article_code = '$old_chain_article'";
    }
    if($prod_desc != $old_prod_desc){
        $addQuery .= ", updated_prod_desc = '$prod_desc', old_prod_desc = '$old_prod_desc'";
    }
    if($mrp != $old_mrp){
        $addQuery .= ",updated_mrp = '$mrp' , old_mrp = '$old_mrp'";
    }
    if($ean != $old_ean){
        $addQuery .= ",updated_ean = '$ean', old_ean = '$old_ean'";
    }
    if($fg_code != $old_fg_code){
        $addQuery .= ",Updated_fg_code = '$fg_code', old_fg_code = '$old_fg_code'";
    }


    $query = "insert into it_product_edit_diary set dealer_item_id = '$productId', master_item_id = '$itemId', by_user =  $curr_userId, ctime = now() $addQuery";
    // print_r($query);
    $id = $this->db->execInsert($query);
    // print_r($id);
    $this->db->closeConnection();
...
function getAllProductList(){
    $this->db = new DBConn();
    // $query = "select id, itemname from it_master_items where itemname != '' order by itemname asc";
    $query = "select id, itemname from it_master_items where itemname != '' and itemname != '0' and product_code != '' order by itemname asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getCityList(){
    $this->db = new DBConn();
    $query = "select distinct dc_city from it_shipping_address WHERE dc_city is not null and dc_city != '0' and dc_city != '' order by dc_city asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getTopThreeSalesCustomers($fromdate,$todate){
    $this->db = new DBConn();
    // use left function in mysql query to take first 10 char only
    $query = "select md.name as customer, sum(drd.tot_amt_including_gst) as sale from it_daily_report_data drd, it_master_dealers md where drd.master_dealer_id = md.id and drd.po_date >= '$fromdate' and drd.po_date <= '$todate' and drd.master_item_description !='' group by drd.master_dealer_id order by sum(drd.tot_amt_including_gst) desc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getTopThreeSalesCategory($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select c.category, sum(drd.tot_amt_including_gst) as sale from it_daily_report_data drd, it_category c where drd.product_category_id = c.id and drd.po_date >= '$fromdate' and drd.po_date <= '$todate' and drd.master_item_description !='' group by drd.product_category_id order by sum(drd.tot_amt_including_gst) desc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getTopThreeSalesProducts($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select master_item_description as product, sum(tot_amt_including_gst) as sale from it_daily_report_data where po_date >= '$fromdate' and po_date <= '$todate' and master_item_description !='' group by master_item_description order by sum(tot_amt_including_gst) desc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getTopThreeSalesRegions($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select zone as region, sum(tot_amt_including_gst) as sale from it_daily_report_data where zone != '' and po_date >= '$fromdate' and po_date <= '$todate' and master_item_description !='' group by zone order by sum(tot_amt_including_gst) desc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getBottomThreeSalesCustomers($fromdate,$todate){
    $this->db = new DBConn();
    // use left function in mysql query to take first 10 char only
    $query = "select md.name as customer, sum(drd.tot_amt_including_gst) as sale from it_daily_report_data drd, it_master_dealers md where drd.master_dealer_id = md.id and drd.po_date >= '$fromdate' and drd.po_date <= '$todate' and drd.master_item_description !='' group by drd.master_dealer_id order by sum(drd.tot_amt_including_gst) asc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getBottomThreeSalesCategory($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select c.category, sum(drd.tot_amt_including_gst) as sale from it_daily_report_data drd, it_category c where drd.product_category_id = c.id and drd.po_date >= '$fromdate' and drd.po_date <= '$todate' and drd.master_item_description !='' group by drd.product_category_id order by sum(drd.tot_amt_including_gst) asc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getBottomThreeSalesProducts($fromdate,$todate){
    $this->db = new DBConn();
    // use left function in mysql query to take first 10 char only
    $query = "select master_item_description as product, sum(tot_amt_including_gst) as sale from it_daily_report_data where po_date >= '$fromdate' and po_date <= '$todate' and master_item_description !='' group by master_item_description order by sum(tot_amt_including_gst) asc
limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
...
function getBottomThreeSalesRegions($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select zone as region, sum(tot_amt_including_gst) as sale from it_daily_report_data where zone != '' and po_date >= '$fromdate' and po_date <= '$todate' and master_item_description !='' group by zone order by sum(tot_amt_including_gst) asc limit 3";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getRegionWisePercentileSale($fromdate,$todate){
    $this->db = new DBConn();
    $query = "select zone, sum(tot_amt_including_gst) as value,round(sum(tot_amt_including_gst) / (select sum(tot_amt_including_gst) from it_daily_report_data where zone != '' and po_date >= '$fromdate' and po_date <= '$todate') * 100,2) as percentage from it_daily_report_data where zone != '' and po_date >= '$fromdate' and po_date <= '$todate' group by zone order by sum(tot_amt_including_gst)";
    // echo $query;
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getDCAddressList(){
    $this->db = new DBConn();
    $query = "SELECT id,dc_address,customer_code FROM it_shipping_address WHERE id IN ( SELECT MAX(id) FROM it_shipping_address GROUP BY dc_address) and dc_address is not NULL";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getStateListByRegion($region){
    $this->db = new DBConn();
    $query = "select id,name from it_regions where zone_id = $region order by name asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getCityListByState($state){
    ...
    $query = "select dc_city from it_shipping_address WHERE dc_city is not null and dc_city != '0' and dc_city != '' and dc_state like '%$state%' group by dc_city order by dc_city asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getStateNameByStateId($stateId){
    $this->db = new DBConn();
    $query = "SELECT name FROM `it_regions` where id = $stateId";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getChainListByCity($city){
    $this->db = new DBConn();

    $query = "select id, name from it_master_dealers where id in (SELECT DISTINCT master_dealer_id FROM `it_shipping_address` WHERE dc_city like '%$city%')";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getDCListByChain($chain,$city){
    
    $this->db = new DBConn();
    $addWhere = "";
    if($city != "" && $city != -1){
        $addWhere .= "and dc_city like '%$city%'";
    }

    $query = "SELECT distinct dc_address, customer_code, id FROM it_shipping_address WHERE master_dealer_id = $chain and dc_address is not NULL $addWhere order by dc_address asc";
...
function getProductsListByChain($category){
    $this->db = new DBConn();
    $query = "select id, itemname from it_master_items where itemname != '' and itemname != '0' and product_code != '' and category_id = $category order by itemname asc";
    $obj = $this->db->fetchObjectArray($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getMasterDealerIdFromDealerName($value){
    $this->db = new DBConn();
    $query = "select id from it_master_dealers where name = '$value'";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getMasterCategoryIdFromCategoryName($value){
    $this->db = new DBConn();
    $query = "select id from it_category where category = '$value'";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getMasterProductIdFromProductName($value){
    $this->db = new DBConn();
    $query = "select id from it_master_items where itemname = '$value'";
    // echo $query;
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
...
function getMasterRegionIdFromRegionName($value){
    $this->db = new DBConn();
    $query = "select id from it_regions where name = '$value'";
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
...
function getVendorMasterDetails($vendorId){
    $this->db = new DBConn();
    $query = "select  v.id,m.displayname,v.vendor_number,v.plant,v.storage_location_code,m.id as mdid from it_vendor_plant_mapping v,it_master_dealers m where v.master_dealer_id=m.id and v.id=$vendorId";
   // echo $query;
   // exit;
   $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
 ...
 function checkVendorMasterDetails($vendorno,$plant,$storage_location_code){
  $this->db = new DBConn();
    $query = "select id from it_vendor_plant_mapping  sh where vendor_number='$vendorno' and plant='$plant' and storage_location_code='$storage_location_code'";
    // echo $query;
    // exit;
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
   ...
   function updateVendorMasterDetails($mdid,$vendorid,$vendorno,$plant,$storage_location_code,$old_vendorno,$old_plant,$old_storage_location_code){
    $this->db = new DBConn();

    $addQuery = "";
   
    if($vendorno != $old_vendorno){
        $addQuery .= ", vendor_number = '$vendorno'";
    }
    if($plant != $old_plant){
        $addQuery .= ", plant = '$plant'";
    }
    if($storage_location_code != $old_storage_location_code){
        $addQuery .= ", storage_location_code = '$storage_location_code'";
    }
//    $addQuery = ltrim($addQuery, ',');

    $query = "update it_vendor_plant_mapping set master_dealer_id= '$mdid' $addQuery where id = $vendorid";
//     print_r($query);
//     exit;
    $id = $this->db->execUpdate($query);
  ...
  function getSkuMasterDetails($sku_id){
    $this->db = new DBConn();
    $query = "select  ekm.id,m.displayname,ekm.sku,ekm.ean,ekm.category,ekm.product_name,ekm.mrp ,ekm.gst ,ekm.inner_size ,ekm.case_size,ekm.outer_size,ekm.purchase_rate_gst,ekm.moq from it_ean_sku_mapping ekm, it_master_dealers m where ekm.master_dealer_id = m.id and ekm.id='$sku_id'";
   $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
  ...
  function checkSkuMasterDetails($sku,$ean,$category,$product_name,$mrp,$gst,$inner_size,$outer_size,$purchase_rate_gst,$moq){
      $this->db = new DBConn();
    $query = "select id from it_ean_sku_mapping s where s.sku='$sku' and s.ean='$ean' and s.category='$category' and s.product_name='$product_name' and s.mrp='$mrp' and s.gst='$gst' and s.inner_size='$inner_size' and s.outer_size='$outer_size' and s.purchase_rate_gst='$purchase_rate_gst' and s.moq='$moq'";
//     echo $query;
//     exit();
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
  ...
  function updateSkuMasterDetails($skuid,$sku,$ean,$category,$product_name,$mrp,$gst,$inner_size,$outer_size,$purchase_rate_gst,$moq,$old_sku,$old_ean,$old_category,$old_product_name,$old_mrp,$old_gst,$old_inner_size,$old_outer_size,$old_purchase_rate_gst,$old_moq){
    $this->db = new DBConn();
    ...
    if($product_name != $old_product_name){
        $addQuery .= ", product_name= '$product_name'";
    }
    if($mrp != $old_mrp){
        $addQuery .= ", mrp= '$mrp'";
    }
    ...
    if($inner_size != $old_inner_size){
        $addQuery .= ", inner_size= '$inner_size'";
    }
//    if($case_size != $old_case_size){
//        $addQuery .= ", case_size= '$case_size'";
//    }
    if($outer_size != $old_outer_size){
        $addQuery .= ", outer_size= '$outer_size'";
    }
    if($purchase_rate_gst != $old_purchase_rate_gst){
        $addQuery .= ", purchase_rate_gst= '$purchase_rate_gst'";
    }
    if($moq != $old_moq){
        $addQuery .= ", moq= '$moq'";
    }
    ...
    $query = "update it_ean_sku_mapping set $addQuery where id = $skuid";
//     print_r($query);
//     exit;
    $id = $this->db->execUpdate($query);
    ...
    return $skuid;
      ...
      function checkEmailDomain($domain){
        $this->db = new DBConn();
    $query = "select id from it_domain d where d.domain='$domain'";
    // echo $query;
    // exit;
    $obj = $this->db->fetchObject($query);
    $this->db->closeConnection();
...
function insertEmailDomain($domain){
      $this->db = new DBConn();
    $query = "insert into it_domain set domain='$domain',ctime=now()";
    // echo $query;
    // exit;
   
    $obj = $this->db->execInsert($query);
    $this->db->closeConnection();
    if(isset($obj)){
        return $obj;
    }else{
...
function getMissingMasterEmailDetails($m_id){
    $this->db = new DBConn();
    $query = "select  id,master_name,emails from it_email_action where id='$m_id'";
   $obj = $this->db->fetchObject($query);
   // echo $query;
   // exit;
    $this->db->closeConnection();
...
function updatemissingmasteremail($mid,$master_name,$emails,$old_emails){
        $this->db = new DBConn();

    $addQuery = "";
    
    if($emails != $old_emails){
        $addQuery .= "emails = '$emails'";
    }

    $query = "update it_email_action set $addQuery where id = $mid";
    // print_r($query);
    // exit;
    $id = $this->db->execUpdate($query);
...
function getPassPolicyDays() {
        $this->db = new DBConn();
        $query = "select * from it_pass_policy_days";
        $obj_policy = $this->db->fetchObject($query);
        $this->db->closeConnection();
        if (isset($obj_policy)) {
            return $obj_policy;
        } else {
