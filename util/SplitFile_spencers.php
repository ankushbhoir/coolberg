...
$dt=date('Y-m-d');
$filePath = DEF_PROCESS_PATH."Spencers_Retail_Limited/newPOs/";
...
//        print_r($data);
      ...
      //  print_r($po_arr);
        ...
        $po_cnt = 0;
         for($i=0;$i<$nolines;$i++){
            $line = trim($num_lines[$i]);
            if(preg_match('/the\s+only\s+proof\s+of\s+delivery\s+by\s+the\s+Company./',$line)==1){
                $po_cnt++;
            }
         }
...
//            if(preg_match('/page\s+\d+\s+of\s+\d+/',$line)==1){
            if(preg_match('/the\s+only\s+proof\s+of\s+delivery\s+by\s+the\s+Company./',$line)==1){
               ...
               $db->execInsert("insert into it_receivedpos set filename=$fname,type='application/pdf',master_dealer_id=8,createtime=now()");
               file_put_contents(DEF_PROCESS_PATH."/Spencers_Retail_Limited/newPOs/".$fileName."_".$dt."_".$cnt.".pdf", $complete_po);
               file_put_contents(DEF_PROCESS_PATH."/Spencers_Retail_Limited/newPOs/".$fileName."_".$dt."_".$cnt.".txt", $complete_po);
