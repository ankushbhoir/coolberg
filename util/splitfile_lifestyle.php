...
$dt=date('Y-m-d');
$filePath = DEF_PROCESS_PATH."LIFESTYLE/newPOs/";
...
//        print_r($data);
      ...
      //  print_r($po_arr);
         ...
         for($i=0;$i<$nolines;$i++){
            $line = trim($num_lines[$i]);
            if(preg_match('/LOCATION:(.+?)\s+GSTIN/', $line)==1){
              echo   $po_cnt++;
            }
         }
...
//            if(preg_match('/page\s+\d+\s+of\s+\d+/',$line)==1){
            ...
            //   echo "$complete_po";
              $po_data .= "$line\n";
               ...
               $db->execInsert("insert into it_receivedpos set filename=$fname,type='application/pdf',master_dealer_id=58,createtime=now()");
